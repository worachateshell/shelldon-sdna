...
class Slot {
    ...
    async spin() {
        ...
        // Create animation with FIXED 10 second duration
        const SPIN_DURATION = 10000; // 10 seconds in milliseconds
...
async function loadUsersFromAPI() {
    try {
        const response = await fetch('/api/guests2');
        if (!response.ok) {
            throw new Error('Failed to fetch users');
        }
        const users = await response.json();
        console.log('Loaded users from API (Users_2):', users);
        return users;
    } catch (error) {
    ...
    drawButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (freshUsers && freshUsers.length > 0) {
                ...
                console.log(`Refreshed ${freshUsers.length} users from Users_2 sheet before draw`);
            }
        } catch (error) {
